...
<head>
    ...
    <style>
        :root { --js-red: #E60013; --js-red-dark: #C40010; --bg-light: #F8FAFC; --text-main: #1E293B; --text-muted: #64748B; --shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1); }
        ...
        .main-container { flex: 1; padding: 40px; max-width: 1400px; }
        .stats-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 25px; margin-bottom: 40px; }
        .stat-card { background: white; padding: 25px; border-radius: 12px; display: flex; align-items: center; gap: 20px; box-shadow: var(--shadow); border-bottom: 4px solid transparent; }
        .icon-wrap { width: 50px; height: 50px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 1.4rem; }
        .card-info h4 { margin: 0; color: var(--text-muted); font-size: 0.85rem; text-transform: uppercase; }
        .card-info .count { font-size: 1.8rem; font-weight: 700; display: block; margin-top: 5px; }
        .stat-card.blue { border-color: #3B82F6; } .stat-card.blue .icon-wrap { background: #DBEAFE; color: #3B82F6; }
        .stat-card.red { border-color: var(--js-red); } .stat-card.red .icon-wrap { background: #FEE2E2; color: var(--js-red); }
        .stat-card.amber { border-color: #F59E0B; } .stat-card.amber .icon-wrap { background: #FEF3C7; color: #F59E0B; }
        
        /* Grid de dos columnas para el contenido inferior */
        .content-columns { display: grid; grid-template-columns: 1fr 1fr; gap: 35px; align-items: start; }
        @media (max-width: 1100px) { .content-columns { grid-template-columns: 1fr; } }
        
        .dashboard-section { margin-bottom: 30px; }
        .item-card { background: white; border-radius: 10px; padding: 20px; display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; border-left: 5px solid #E2E8F0; transition: 0.3s; box-shadow: 0 1px 2px rgba(0,0,0,0.05); }
        .item-card:hover { transform: translateX(5px); box-shadow: var(--shadow); }
        .item-card.priority { border-left-color: var(--js-red); }
        .item-main { display: flex; flex-direction: column; gap: 4px; }
        .item-name { font-weight: 700; font-size: 1.05rem; color: var(--text-main); }
        .btn-action { background: #F1F5F9; color: var(--text-main); padding: 8px 18px; border-radius: 6px; text-decoration: none; font-size: 0.9rem; font-weight: 600; display: flex; align-items: center; gap: 8px; transition: 0.2s; }
...
<body>
    ...
    <header class="top-header">
        ...
        <div class="user-nav"><span style="font-weight: 500;"><?php echo date('d M, Y'); ?></span><a href="../logout.php" class="btn-exit"><i class="fas fa-sign-out-alt"></i> Salir</a></div>
    </header>
    ...
    <div class="admin-wrapper">
        ...
        <main class="main-container">
            <?php if ($status_msg): ?>
                <div style="background: <?php echo ($status_type === 'success') ? '#DCFCE7' : '#FEE2E2'; ?>; color: <?php echo ($status_type === 'success') ? '#166534' : '#991B1B'; ?>; padding: 15px; border-radius: 8px; margin-bottom: 25px; font-weight: 500; display: flex; align-items: center; gap: 10px;">
            ...
            <div class="page-title" style="margin-bottom: 30px;">
                ...
                <p style="color: var(--text-muted); margin-top: 5px;">Gestiona las solicitudes de alumnos y empresas.</p>
            </div>

            <div class="stats-grid">
                <div class="stat-card blue"><div class="icon-wrap"><i class="fas fa-user-graduate"></i></div><div class="card-info"><h4>Alumnos</h4><span class="count"><?php echo $count_alumnos; ?></span></div></div>
                <div class="stat-card red"><div class="icon-wrap"><i class="fas fa-briefcase"></i></div><div class="card-info"><h4>Vacantes</h4><span class="count"><?php echo $count_vacantes; ?></span></div></div>
                <div class="stat-card amber"><div class="icon-wrap"><i class="fas fa-building"></i></div><div class="card-info"><h4>Empresas</h4><span class="count"><?php echo $count_pendientes; ?></span></div></div>
                <div class="stat-card blue" style="border-color: #2563eb;"><div class="icon-wrap" style="background:#DBEAFE; color:#2563eb;"><i class="fas fa-clipboard-check"></i></div><div class="card-info"><h4>Trámites SSPP</h4><span class="count"><?php echo $count_sspp; ?></span></div></div>
            </div>
            ...
            <div class="content-columns">
                ...
                <div class="column-left">
                    <div class="dashboard-section">
                        <h3 style="font-family: 'Montserrat'; font-size: 1.2rem; color: var(--text-main); border-bottom: 2px solid #F1F5F9; padding-bottom: 10px; margin-bottom: 20px;">
                            <i class="fas fa-check-circle" style="color: #10B981;"></i> Directorio de Empresas Activas
                        </h3>
                        
                        <?php if (empty($empresas_activas)): ?>
                            <div style="background: white; padding: 25px; text-align: center; border-radius: 12px; color: var(--text-muted); border: 1px dashed #CBD5E1;">Aún no hay empresas aprobadas.</div>
                        <?php else: ?>
                            <?php foreach ($empresas_activas as $activa): ?>
                                <div class="item-card" style="border-left-color: #10B981;">
                                    <div class="item-main">
                                        <span class="item-name"><?php echo htmlspecialchars($activa['nombre_empresa']); ?></span>
                                        <span style="font-size: 0.85rem; color: var(--text-muted);"><i class="fas fa-envelope"></i> <?php echo htmlspecialchars($activa['email_contacto']); ?></span>
                                        ...
                                        if ($activa['es_catalogo_sspp'] && !empty($activa['vigencia_sspp'])) {
                                            ...
                                            if ($diferencia->invert) {
                                                ...
                                                $color_bg = "#FEE2E2"; $color_txt = "#991B1B"; $icono = "fa-times-circle";
                                            } else {
                                                if ($diferencia->y > 0) {
                                                ...
                                                $color_bg = "#ECFDF5"; $color_txt = "#059669"; $icono = "fa-hourglass-half";
                                            }
                                    ...
                                    <a href="gestionar_empresa.php?id=<?php echo $activa['id_empresa']; ?>" class="btn-action" style="color: #10B981; background: #ECFDF5;"><i class="fas fa-building"></i></a>
                                </div>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </div>
                ...
                <div class="column-right">
                    <div class="dashboard-section">
                        ...
                        <?php if (empty($tramites_sspp)): ?>
                            <div style="background: white; padding: 25px; text-align: center; border-radius: 12px; color: var(--text-muted); border: 1px dashed #CBD5E1;">No hay trámites activos en este momento.</div>
                        <?php else: ?>
                            <?php foreach ($tramites_sspp as $tramite): ?>
                                <div class="item-card priority">
                                    <div class="item-main">
                                        <span class="item-name"><?php echo htmlspecialchars($tramite['nombre_empresa']); ?></span>
                                        <div style="display: flex; gap: 15px; align-items: center; margin-top: 5px;">
                                            <span class="badge <?php echo ($tramite['estado_tramite'] == 'Datos Recibidos') ? 'bg-green' : 'bg-yellow'; ?>"><?php echo $tramite['estado_tramite']; ?></span>
                                        </div>
                                    </div>
                                    <a href="detalle_tramite_sspp.php?id=<?php echo $tramite['id_solicitud']; ?>" class="btn-action" style="background: var(--js-red); color: white;"><i class="fas fa-arrow-right"></i></a>
                                </div>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="dashboard-section">
                        ...
                        <?php foreach ($empresas_pendientes as $empresa): ?>
                            <div class="item-card" style="border-left-color: #F59E0B;">
                                <div class="item-main">
                                    <span class="item-name"><?php echo htmlspecialchars($empresa['nombre_empresa']); ?></span>
                                    <span style="font-size: 0.85rem; color: var(--text-muted);"><i class="fas fa-envelope"></i> <?php echo htmlspecialchars($empresa['email_contacto']); ?></span>
                                </div>
                                <a href="gestionar_empresa.php?id=<?php echo $empresa['id_empresa']; ?>" class="btn-action"><i class="fas fa-eye"></i></a>
                            </div>
                        <?php endforeach; ?>
                    </div>
                    <?php endif; ?>
