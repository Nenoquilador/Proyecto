...
<body>
    ...
    <div class="dashboard-grid">
        ...
        <main class="main-content">            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
                <h2 style="margin: 0; font-size: 1.8rem; font-weight: 800; color: var(--text);">Trámites SSPP</h2>
            </div>

            <form method="GET" action="gestionar_tramites.php" style="margin-bottom: 24px; display: flex; gap: 16px;">
                <div class="search-bar" style="flex: 1;">
                    <i class="fas fa-search"></i>
                    <input type="text" name="search" placeholder="Buscar por nombre de empresa..." value="<?php echo htmlspecialchars($search); ?>">
                </div>
                <select name="estado" class="form-select" style="width: 250px; padding: 12px; border-radius: 8px; border: 1px solid var(--border);">
                    <option value="">Todos los Estados</option>
                    <option value="Solicitud Inicial" <?php if($filtro_estado == 'Solicitud Inicial') echo 'selected'; ?>>Solicitud Inicial</option>
                    <option value="Formato Enviado" <?php if($filtro_estado == 'Formato Enviado') echo 'selected'; ?>>Formato Enviado</option>
                    <option value="Datos Recibidos" <?php if($filtro_estado == 'Datos Recibidos') echo 'selected'; ?>>Datos Recibidos</option>
                    <option value="Validado por Teléfono" <?php if($filtro_estado == 'Validado por Teléfono') echo 'selected'; ?>>Validado por Teléfono</option>
                    <option value="Aprobado Catálogo" <?php if($filtro_estado == 'Aprobado Catálogo') echo 'selected'; ?>>Aprobado Catálogo</option>
                </select>
            ...
            <div class="premium-form-card" style="padding: 0; overflow: hidden;">
                <?php if(empty($tramites)): ?>
                    <div class="empty-state" style="padding: 60px 20px; text-align: center;">
                        <i class="fas fa-file-signature" style="font-size: 3rem; color: var(--border-light); margin-bottom: 16px;"></i>
                        <p>No se encontraron trÃ¡mites con los criterios seleccionados.</p>
                    </div>
                <?php else: ?>
                    <table style="width: 100%; border-collapse: collapse;">
                        <thead style="background: var(--surface-alt);">
                            <tr>
                                <th style="padding: 16px; text-align: left; font-size: 0.85rem; color: var(--text-secondary); text-transform: uppercase;">Institución / Empresa</th>
                                <th style="padding: 16px; text-align: left; font-size: 0.85rem; color: var(--text-secondary); text-transform: uppercase;">Fase del Trámite</th>
                                <th style="padding: 16px; text-align: left; font-size: 0.85rem; color: var(--text-secondary); text-transform: uppercase;">Fecha Inicio</th>
                                <th style="padding: 16px; text-align: left; font-size: 0.85rem; color: var(--text-secondary); text-transform: uppercase;">Última Validación</th>
                                <th style="padding: 16px; text-align: left; font-size: 0.85rem; color: var(--text-secondary); text-transform: uppercase;">Acción</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                <td style="padding: 16px; border-bottom: 1px solid var(--border-light); font-weight: 500; color: var(--text);">
                                    <?php echo htmlspecialchars($t['nombre_empresa']); ?>
                                </td>
                                <td style="padding: 16px; border-bottom: 1px solid var(--border-light);">
                                    <span class="status-badge <?php echo obtenerClaseEstado($t['estado_tramite']); ?>" style="padding: 4px 10px; border-radius: 6px; font-size: 0.8rem; font-weight: 600;">
                                ...
                                <td style="padding: 16px; border-bottom: 1px solid var(--border-light); color: var(--text-secondary); font-size: 0.9rem;">
                                    <?php echo htmlspecialchars(date('d/m/Y', strtotime($t['fecha_inicio']))); ?>
                                </td>
                                <td style="padding: 16px; border-bottom: 1px solid var(--border-light); color: var(--text-secondary); font-size: 0.9rem;">
                                    <?php echo $t['fecha_validacion'] ? htmlspecialchars(date('d/m/Y', strtotime($t['fecha_validacion']))) : 'Pendiente'; ?>
                                </td>
                                <td style="padding: 16px; border-bottom: 1px solid var(--border-light);">
                                    <a href="detalle_tramite.php?id=<?php echo $t['id_solicitud']; ?>" class="btn-primary" style="padding: 6px 12px; font-size: 0.8rem; text-decoration: none;">
                                        Gestionar
                                    </a>
                                </td>
                            </tr>
